...
- **Test plant** — `schema/warehouse.example.json` + `.content.example.json`: a small demo
  plant (default, loaded on start), generated by `scripts/generate-example-warehouse.js`.
- **CML** — `schema/CML.plan.json` + `CML.content.json`: Batiment 13A of a real plant, 453
  locations, imported from the plant's Excel floor plan by
