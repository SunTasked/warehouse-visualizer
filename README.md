...
- **CML** — `schema/CML.plan.json` + `CML.content.json` + `CML.picking-lists.json`: building
  13A of a real plant, 453 locations (empty stock — its picking lists receive before they
  pick), with the plan imported from the plant's Excel floor plan by
  `python scripts/import_plan_xlsx.py --building "BATIMENT 13A"` (needs `openpyxl`). The
  workbook itself lives in `data/`, which is gitignored — it carries real operational
  records. See specs.md §5.7 for how the plan is read.
