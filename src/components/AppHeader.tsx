...
import { useAnalytics, type AppTab } from "../state/AnalyticsContext";
import { useViewFocus } from "../state/ViewFocusContext";
import { WAREHOUSE_PRESETS, type WarehousePreset } from "../data/presets";
import { openContentFile, openJsonFile, type FileHandle } from "../lib/file";
import { isStockExtract, parseStockExtract } from "../lib/stockExtract";
import type { WarehouseConfig, WarehouseContent } from "../types/warehouse";
import type { PickingListsFile } from "../types/simulation";
import { ContentLoadDialog, readContentLoad, type PendingContent } from "./ContentLoadDialog";
...
/**
 * Title, warehouse identity, the three top-level tabs and the slot search
 * (specs.md §5.6). Performances is gated on there being something to look at
 * — the board is meaningless without a recorded session, and a disabled tab
 * says that more plainly than an empty board would. Statistics always has the
 * stock on screen to count.
 */
...
Continue?`);
  ...
  /**
   * Content is either content JSON or a stock extract straight from the WMS
   * (src/lib/stockExtract.ts). Either loads at once when there is nothing to
   * say about it; otherwise the dialog says it — slots the plan doesn't have,
   * slots given more than they hold, stock about to be replaced.
   */
  const loadContent = async () => {
    setShowMenu(false);
    let opened;
    try {
      opened = await openContentFile();
    } catch (err) {
      console.error(err);
      window.alert("That file couldn't be opened.");
      return;
    }
    if (!opened) return;

    let pending: PendingContent;
    if (isStockExtract(opened.name, opened.text)) {
      try {
        pending = { kind: "extract", fileName: opened.name, extract: parseStockExtract(opened.text) };
      } catch (err) {
        window.alert(err instanceof Error ? err.message : "That extract couldn't be read.");
        return;
      }
    } else {
      let content: WarehouseContent;
      try {
        content = JSON.parse(opened.text) as WarehouseContent;
      } catch {
        window.alert("That file couldn't be read as JSON.");
        return;
      }
      ...
      pending = { kind: "json", fileName: opened.name, content, handle: opened.handle };
    }

    const load = readContentLoad(pending, warehouse, hasStock);
    if (load.clean) applyContent(load.content, load.handle);
    else setPendingContent(pending);
  };

  const cancelContent = useCallback(() => setPendingContent(null), []);
  ...
  const loadItems = [
    ...
    {
      key: "content",
      label: "Warehouse content…",
      description: "Stock: a content file, or a pallet count extract (CSV)",
      run: loadContent,
    },
    { key: "lists", label: "Picking lists…", description: "Work orders for the forklift", run: loadPickingLists },
  ];

  const tabClass = (tab: AppTab) => (analytics.tab === tab ? "app__tab app__tab--active" : "app__tab");
    ...
    <header className="app__header">
      <div className="app__header-main">
        ...
        <nav className="app__tabs">
          ...
          <button
            ...
            title="Capacity and fill of the stock on screen, for the plant and each building"
          >
            Statistics
          </button>
          <button
      ...
      {pendingContent && (
        <ContentLoadDialog
          ...
          onLoad={(content, handle) => {
            setPendingContent(null);
            applyContent(content, handle);
          }}
        />
      )}
    </header>
